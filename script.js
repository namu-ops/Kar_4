...
function initializeForms() {
    ...
    if (bookingForm) {
        ...
        if (dateInput) {
            ...
            // Если значение по умолчанию в прошлом, устанавливаем завтрашнюю дату
            const currentValue = new Date(dateInput.value);
        ...
        bookingForm.addEventListener('submit', function(e) {
            ...
            // В реальном приложении здесь был бы fetch запрос на сервер
            ...
            // Очистка формы через 5 секунд (в реальном приложении не очищаем)
            setTimeout(() => {
                formMessage.style.display = 'none';
                // Не очищаем форму, чтобы пользователь видел свои данные
            }, 5000);
        });
    }
...
function initializeSmoothScroll() {
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            ...
            if (targetElement) {
                // Для мобильного меню закрываем его при переходе
                const mobileMenu = document.querySelector('.mobile-menu');
